...
// ── Home Screen ─────────────────────────────────────────────────────────────
function HomeScreen({ name, role, count, observations, onUpload, onMap, onMyRecords, onSettings }: {
  name: string; role: Role; count: number; observations: PlantObservation[];
  onUpload: () => void; onMap: () => void; onMyRecords: () => void; onSettings: () => void;
}) {
  const GOAL = 5;
...
// ── Settings Screen ──────────────────────────────────────────────────────────
function SettingsScreen({ onBack, onLogout, darkMode, onToggleDarkMode, soundEnabled, onToggleSound, currentUser, onUpdateUser }: {
  onBack: () => void;
  onLogout: () => void;
  darkMode: boolean;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pb-10">
      ...
      <div className="px-5 mt-2 space-y-3">
        ...
        <button onClick={onLogout} className="w-full bg-card rounded-2xl border border-border shadow-sm flex items-center gap-4 p-4 hover:bg-destructive/5 transition-colors">
          <div className="w-10 h-10 bg-destructive/10 rounded-xl flex items-center justify-center shrink-0">
            <LogOut size={18} className="text-destructive" />
          </div>
          <p className="font-bold text-sm text-destructive" style={{ fontFamily: "Nunito, sans-serif" }}>로그아웃</p>
        </button>
      </div>
    </div>
  );
